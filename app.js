...
function renderOeeMachineEditor(rows=state.oeeMachineEditorData){
  ...
  const order=oee986MachineOrder();
  ...
  wrap.innerHTML=`
    <div class="oee-editor-head">
      ...
      <span class="muted">
        Só valores confirmados pela IA visual na mesma célula podem alimentar o relatório.
      </span>
    </div>
    ...
    <div class="oee-editor-grid">
      ${state.oeeMachineEditorData.map((row,index)=>{
        ...
        const preview=state.oeeRowPreviews?.[index]||'';

        const statusClass=
          displayed!==''?'confidence-good':
          row.status==='not_running' || row.status==='blank'
            ?'confidence-neutral'
            :'confidence-empty';

        const statusText=
          displayed!==''
            ?`${String(displayed).replace('.',',')}% — confirmado`
            :row.status==='not_running'
              ?'Não rodou / sem apontamento'
              :row.status==='blank'
                ?'Célula vazia'
                :'Não confirmado';

        return `
          <label class="oee-editor-row ${statusClass}">
  ...
  $$('.oee-editor-input').forEach(input=>{
    const confirm=event=>{
      ...
      oee986BlockUnsafeReport(state.oeeMachineEditorData);
    };
...
async function analyzeOeeWithVision(
  ...
  rows._diagnostic={
    ...
    model:data.model||'',
    api:data.api||'',
    confirmed:oee986ConfirmedRows(rows).length
  };
...
// =====================================================
// V98.6 — LEITOR ESTRUTURAL DE QUADRO OEE
// =====================================================

function oee986ScopeLabel(dateValue, shiftValue){
  try{
    const scope=boardScopeForReport(dateValue,shiftValue);
    return scope?.label||String(shiftValue||'');
  }catch(_){
    return String(shiftValue||'');
  }
}

function oee986MachineOrder(){
  return Array.isArray(OEE_BOARD_MACHINES)
    ?OEE_BOARD_MACHINES
    :[
      'MK-138','MK-105','MK-108','MK-223','MK-192',
      'MK-69','MK-172','MK-173','MK-178','MK-179',
...
function oee986NormalizeMachine(value){
...
function oee986BuildOriginalCells(image, operationalDate, shift){
  const naturalWidth=image.naturalWidth||image.width;
  const naturalHeight=image.naturalHeight||image.height;

  // Usa o recorte legado APENAS para obter a coluna correspondente.
  const crop=getLegacyOeeCropSettings(
    image,
    operationalDate,
    shift
  ...
  // Cria referência da coluna para detectar as linhas horizontais.
  const ref=document.createElement('canvas');
  const rctx=ref.getContext('2d',{willReadFrequently:true});

  ref.width=Math.max(700,Math.min(1100,Math.round(crop.sw)));
  ref.height=Math.max(
    1400,
    Math.round(crop.sh*(ref.width/Math.max(1,crop.sw)))
  );
  ...
  let ranges=[];
  try{
    ranges=machineRowRangesFromCanvas(ref)||[];
  }catch(_){
    ranges=[];
  }

  const machines=oee986MachineOrder();

  return machines.map((machine,index)=>{
    const fallbackTop=index*(ref.height/machines.length);
    const fallbackBottom=(index+1)*(ref.height/machines.length);
    ...
    const topRatio=Math.max(
      0,
      Math.min(1,Number(range.top)/ref.height)
    );
    const bottomRatio=Math.max(
      topRatio,
      Math.min(1,Number(range.bottom)/ref.height)
    );

    const baseY=crop.sy+crop.sh*topRatio;
    const baseBottom=crop.sy+crop.sh*bottomRatio;
    const baseH=Math.max(1,baseBottom-baseY);

    // Margem generosa, mas sem invadir muito as linhas vizinhas.
    const padY=Math.min(
      Math.max(5,baseH*0.16),
      baseH*0.22
    );

    // Mais largura da célula porque muitos percentuais ficam no extremo direito.
    const padX=Math.max(6,crop.sw*0.16);

    const sx=Math.max(0,crop.sx-padX);
    const sy=Math.max(0,baseY-padY);
    const ex=Math.min(naturalWidth,crop.sx+crop.sw+padX);
    const ey=Math.min(naturalHeight,baseBottom+padY);
    ...
    // Alta resolução real sem criar uma "faixa".
    const targetW=Math.max(1100,Math.min(2000,Math.round(sw*2.0)));
    const aspect=sh/Math.max(1,sw);
    const targetH=Math.max(260,Math.min(620,Math.round(targetW*aspect*1.15)));
...
function oee986MergeVisionRows(visionRows){
  const map=new Map(
    (visionRows||[]).map(row=>[
      oee986NormalizeMachine(row.machine),
      row
    ])
  );
...
// =====================================================
// V98.9 — PÓS-LEITURA APENAS
// A leitura da foto permanece EXATAMENTE a da V98.7.
// =====================================================

function v989PriorityCandidates(){
  return (state.oeeMachineEditorData||[])
    .filter(row=>{
...
function v989SelectedMachines(){
  state.v989SelectedPriorities=
    Array.isArray(state.v989SelectedPriorities)
      ?state.v989SelectedPriorities
      :[];

  return state.v989SelectedPriorities.slice(0,3);
}

function v989TogglePriority(machine){
  ...
  const pos=state.v989SelectedPriorities.indexOf(mk);

  if(pos>=0){
    state.v989SelectedPriorities.splice(pos,1);
  }else{
    if(state.v989SelectedPriorities.length>=3){
      showToast('Escolha no máximo 3 prioridades.');
      return;
    }
    state.v989SelectedPriorities.push(mk);
  }

  v989RenderPriorityPicker();
}

function v989RenderPriorityPicker(){
  const box=
    $('automaticPriorities')||
    $('priorityList')||
    $('prioritiesList');
  ...
  const candidates=v989PriorityCandidates();
  const allowed=new Set(candidates.map(r=>r.machine));

  state.v989SelectedPriorities=
    v989SelectedMachines()
      .filter(m=>allowed.has(m));

  if(!candidates.length){
    box.innerHTML=`
      <div class="empty-state">
        Nenhuma máquina abaixo de 65% no OEE confirmado.
      </div>
    `;
    return;
  }
  ...
  box.innerHTML=`
    <div class="v989-top10-head">
      <strong>Top 10 prioridades por OEE</strong>
      <span>Escolha somente 3 para o próximo turno.</span>
    </div>

    <div class="v989-top10-list">
      ${candidates.map((row,index)=>{
        const selected=state.v989SelectedPriorities.includes(row.machine);
        const oee=Number(row.oee);
        ...
        return `
          <button
            type="button"
            class="v989-priority ${selected?'selected':''}"
            data-machine="${escapeHtml(row.machine)}"
          >
            <span class="v989-rank">${index+1}</span>

            <span class="v989-main">
              <strong>${escapeHtml(row.machine)}</strong>
              <span>
                OEE ${oee.toFixed(1).replace('.',',')}%
                • perda ${loss.toFixed(1).replace('.',',')} h
              </span>
            </span>

            <span class="v989-check">
              ${selected?'✓':'○'}
            </span>
          </button>
        `;
    ...
    <div class="v989-count">
      ${state.v989SelectedPriorities.length}/3 selecionada(s)
    </div>
  `;

  $$('.v989-priority').forEach(btn=>{
    btn.addEventListener('click',()=>{
      v989TogglePriority(btn.dataset.machine);
    });
  });
}

function v989GetSelectedRows(){
  const byMachine=new Map(
    v989PriorityCandidates()
      .map(row=>[row.machine,row])
  );

  return v989SelectedMachines()
    .map(m=>byMachine.get(m))
    .filter(Boolean)
    .slice(0,3);
}

function v989PreviousShiftLabel(scopeLabel=''){
...
function v989FindShiftOee(label){
  ...
  for(const pool of pools){
    ...
    for(const item of arr){
      ...
      if(itemLabel===target && Number.isFinite(value)){
        return value;
      }
    }
  }
...
function v989TrendText(currentOee,scopeLabel){
  ...
  const prevLabel=v989PreviousShiftLabel(scopeLabel);
  const previous=v989FindShiftOee(prevLabel);

  if(!prevLabel || !Number.isFinite(previous)){
    return 'Tendência da eficiência: turno anterior não disponível.';
  }
...
function v989CompactActionBlock(){
  const rows=v989GetSelectedRows();

  if(rows.length<3){
    return '*AÇÕES PARA CORREÇÃO*\nSelecione 3 prioridades no Top 10.';
  }
...
function maintenanceMessage(){
  ...
  const supervisorRows=fusionRows
  ...
  const recurrence=deriveRecurrenceMachines(state.analysis)
    .filter(machine=>
      supervisorRows.some(row=>
        row.machine===normalizeMachineCode(machine)
      )
    );
  ...
  // Usa a mesma lista persistida no relatório, mesmo após trocar de tela/recarregar.
  const photoOeeRows=v991PrioritySourceRows()
    .map(row=>({
      machine:row.machine,
      oee:Number(row.oee),
      description:'OEE confirmado na leitura do quadro.'
    }))
    .filter(row=>
      Number.isFinite(row.oee) &&
      row.oee>=0 &&
  ...
  if(photoOeeRows.length){
    ...
    lines.push(
      `Leitura da foto: ${photoOeeRows.length} confirmado(s).`
    );
  }
  ...
  if(supervisorRows.length!==3){
    lines.push(
      'Selecione exatamente 3 máquinas no Top 10 para definir as ações do próximo turno.'
    );
  }else{
  ...
  if(recurrence.length){
    lines.push(
      `Reincidência entre as prioridades atuais: ${recurrence.join(', ')}.`
    );
  }

  lines.push(
    '*Foco:* resolver as três prioridades escolhidas durante o turno, registrar no SGMan e evitar retrabalho.'
  );
...
// =====================================================
// V99.1 — TOP 10 USA EXATAMENTE O OEE DO RELATÓRIO
// =====================================================

function v991CurrentScopeKey(){
  return [
    String(state.analysis?.date||$('reportDate')?.value||''),
    String(state.analysis?.shift||$('reportShift')?.value||'1')
  ].join('|');
}
...
function v991ManageSelectionScope(){
  const key=v991CurrentScopeKey();
  const storageKey='turnosmart_supervisor_priority_scope_v991';
...
function supervisorFusionRanking(limit=10){
  ...
  for(const current of v991PrioritySourceRows()){
    const machine=current.machine;
    const oee=Number(current.oee);
  ...
  return applyAutomaticSupervisorSelection(
    rows
      .sort((a,b)=>
        a.oee-b.oee ||
...
function renderSupervisorFusionPanel(){
  ...
  target.innerHTML=`
    <div class="supervisor-confirm-box">
      ...
      <p>
        As máquinas abaixo são os mesmos OEE confirmados usados no relatório atual.
        Escolha somente 3 para o próximo turno.
      </p>
    ...
    <div class="supervisor-fusion-panel">
      ${
        rows.length
          ...
          `).join('')
          :'<p class="muted">Nenhum OEE confirmado no relatório atual. Analise a foto primeiro.</p>'
      }
    </div>
  `;
  ...
  $$('.supervisor-priority-check').forEach(input=>{
    input.addEventListener('change',()=>{
      ...
      const selectedMachines=checked
...
async function analyzeCurrentReport() {
  ...
  try {
    ...
    // V99.1: o Top 10 usa exatamente analysis.machineOee.
    // Não apaga as 3 escolhas ao atualizar o mesmo turno.
    v991ManageSelectionScope();
    ...
    state.supervisorFusionRows=
      applyAutomaticSupervisorSelection(
        state.supervisorFusionRows
      );
